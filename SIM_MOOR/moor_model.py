...
import os
import shutil
from .moor_background import Ocean
from .moor_utility import auto_connect_objects, connect_objects, runge_kutta4
...
class Model():
    ...
    def add(self, moor_obj):
        ...
        if moor_obj.name in self.moor_obj_dict:
            raise ValueError("the name \"{}\" is already in use.".format(moor_obj.name))
        ...
        for time_step in range(num_time_steps):
            ...
            if (time_step % save_time_steps == 0):
                ...
                for obj in moor_obj_list:
                    self.save_node_data_csv(obj, present_time, dir_name, save_header)
                    save_header = 1
    ...
    def save_node_data_csv(self, obj, present_time, dir_name, save_header):
        ...
        file_name = './'+ dir_name + '/' + 'node_data'+ str("%.5f" % (present_time))+'.csv'
        ...
        if save_header == 0:
            ...
            dataframe.to_csv(file_name, sep=',', mode='a', header=False, columns = title_list)
    ...
    def summary(self):
        ...
        print ( ("{:^"+table_width_str+"s}|{:^"+table_width_str+"s}|").format( "Name", "N_elements") )

        for _, moor_obj in self.moor_obj_dict.items():
            print ( ("{:^"+table_width_str+"s}|{:^"+table_width_str+"d}|").format( moor_obj.name, moor_obj.num_element ) )
